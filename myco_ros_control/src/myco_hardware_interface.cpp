#include <myco_ros_control/myco_hw_interface.h>
#include <myco_ros_control/visibility_control.h>
...
namespace myco_hardware_interface{
  ...
  CallbackReturn MycoHWInterface::on_init(const hardware_interface::HardwareInfo & info)
  {
    ...
    n_->declare_parameter("myco_ethercat_drivers",std::vector<std::string>({"myco"}));
    n_->get_parameter_or("myco_ethercat_drivers",myco_driver_names_, myco_driver_names_default);
    std::string ethernet_name;
    n_->declare_parameter("myco_ethernet_name","eth0");
    n_->get_parameter_or("myco_ethernet_name", ethernet_name,ethercat_name_default);
    ethercat_drivers_.clear();
    ethercat_drivers_.resize(myco_driver_names_.size());

    em = new myco_ethercat_driver::EtherCatManager(ethernet_name);
    for(unsigned int i=0;i<ethercat_drivers_.size();i++)
    {
      ethercat_drivers_[i] = new myco_ethercat_driver::MycoEtherCATDriver(em,myco_driver_names_[i],n_);
    }
    module_infos_.clear();
  ...
  std::vector<hardware_interface::StateInterface> MycoHWInterface::export_state_interfaces()
  {
  ...
  std::vector<hardware_interface::CommandInterface> MycoHWInterface::export_command_interfaces()
  {
  ...
  return_type MycoHWInterface::prepare_command_mode_switch(const std::vector<std::string>& start_interfaces, const std::vector<std::string>& stop_interfaces)
  {
  ...
  return_type MycoHWInterface::perform_command_mode_switch(const std::vector<std::string>&, const std::vector<std::string>&)
  {
  ...
  return_type MycoHWInterface::read(const rclcpp::Time &time, const rclcpp::Duration &period)
  {
    rclcpp::spin_some(n_);
    for(size_t i=0;i<module_infos_.size();i++)
    {
  ...
  return_type MycoHWInterface::write(const rclcpp::Time &time, const rclcpp::Duration &period)
  {
    // rclcpp::spin_some(n_);
  ...
  CallbackReturn MycoHWInterface::on_activate(const rclcpp_lifecycle::State & previous_state)
  {
  ...
  CallbackReturn MycoHWInterface::on_deactivate(const rclcpp_lifecycle::State & previous_state)
  {
...
PLUGINLIB_EXPORT_CLASS(myco_hardware_interface::MycoHWInterface,
